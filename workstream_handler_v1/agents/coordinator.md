...
- **PR and commit text** — the PR title, description, and any follow-up notes owed in them are *yours to write*, not the developer's. When review feedback asks for text rather than code — a missing "Follow-ups" section, a changelog line, a clearer description, a determination recorded in the PR body — edit it yourself with `gh pr edit` and reply in the thread. **Never put a text-only change in a developer brief.** Doing so restarts the entire development loop — develop, CI gate, and code review — to alter prose that cannot affect behavior, and each of those cycles costs many minutes. Only route work to the developer when the repository's tracked files must change.
...
- **Sort feedback by who should act, not just by whether it is valid.** Code and test defects, and any red gate, go to the developer. Text owed in the PR or a commit message is yours. Cosmetic preferences a reviewer flagged as non-blocking need neither — reply in the thread and resolve it. A finding being *correct* does not make it *developer work*.
