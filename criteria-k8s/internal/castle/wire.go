...
func lifecycleFromEnvelope(env *v1.Envelope) (events.LifecycleEvent, bool) {
	...
	// environment the adapter runs in — the per-scope reconcile's
	// co-location grouping key (CRI-234). Empty keeps the per-adapter pod
	// fallback for older engines.
	ev.Environment = dataString(data, "environment", "environment_id")
